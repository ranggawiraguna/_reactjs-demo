...
class Backend_Put extends React.Component {
  ...
  putDataUser = async (path, id, data) => {
    let success = API.putUserData(path, id, data);

    if (success) {
      alert("Data berhasil di update");
      document
        .querySelector(
          '#GlobalAPI_Section.container-context .container-operation #ModalUpdate .modal-header button[data-dismiss="modal"]'
        )
        .click();
  ...
  changeNavOperation(elemenSelected) {
    document.querySelector(
      "#GlobalAPI_Section.container-context .container-operation .dropdown button"
    ).innerHTML = elemenSelected.innerHTML;
    document
      .querySelectorAll(
        "#GlobalAPI_Section.container-context .container-operation .dropdown-item"
      )
      .forEach((e) => {
  ...
  componentDidMount = async () => {
    ...
    document
      ...
      .forEach((item) =>
        item.addEventListener("click", () => this.changeNavOperation(item))
      );
      ...
      .forEach((btn) => {
        btn.addEventListener("click", () => {
          this.changeUpdateForm(
            ...
            parseInt(
              btn.parentElement.parentElement.querySelector("th").innerHTML - 1
            ),
            btn.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.getAttribute(
              "aria-labelledby"
            )
          );
        });
    ...
    document
      .querySelector(
        "#GlobalAPI_Section.container-context .container-operation #ModalUpdate .modal-footer button:last-child"
      )
      .addEventListener("click", async (e) => {
        let modalContent = e.target.parentElement.parentElement;
        let newData = { id: modalContent.dataset.id };

        modalContent.querySelectorAll("#ModalBody span.form-control").forEach(
          (input) =>
            (newData = {
              ...newData,
              [input.getAttribute("name")]:
                input.getAttribute("name") === "progress"
                  ? parseInt(input.innerHTML)
                  : input.innerHTML,
            })
        );

        if (
          await this.putDataUser(
            modalContent.dataset.path,
            modalContent.dataset.id,
            newData
          )
        ) {
          this.setState({ user: await this.getDataUser() });
  ...
  render() {
    return (
      <Frame>
        <Container id="GlobalAPI_Put">
          <Header2 innerValue="Global API - PUT" />
          <hr />
          <ContainerContext className="crud">
            <ContainerCRUD>
              ...
              <ContainerOperation>
                ...
                <ModalUpdate
                  dataUpdate={this.state.updateDetails.data}
                  pathUpdate={this.state.updateDetails.path}
                  idUpdate={this.state.updateDetails.id}
                  titleUpdate={this.state.updateDetails.title}
                  columnUpdate={this.state.updateDetails.column}
                />
              </ContainerOperation>
