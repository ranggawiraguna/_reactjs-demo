...
class Backend_Post extends React.Component {
  ...
  postDataUser = async (path = "", data = {}) => {
    let success = await API.postUserData(path, data);
  ...
  componentDidMount = async () => {
    ...
    document
      .querySelector(
        "#GlobalAPI_Section.container-context .container-operation .tab-pane > div > span > .input-range > input"
      )
      .addEventListener("change", (e) => {
        document.querySelector(
          "#GlobalAPI_Section.container-context .container-operation .tab-pane > div > span > .input-range > span > p"
        ).innerHTML = e.target.value;
      });

    document
      .querySelectorAll(
        "#GlobalAPI_Section.container-context .container-operation .tab-pane > div > button"
      )
      .forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          btn.parentElement
            ...
            .forEach(
              (input) => (newData = { ...newData, [input.name]: input.value })
            );
          if (
            await this.postDataUser(
              btn.parentElement.getAttribute("name"),
              newData
            )
          ) {
            this.setState({ user: await this.getDataUser() });
  ...
  render() {
    return (
      <Frame>
        <Container id="GlobalAPI_Post">
          <Header2 innerValue="Global API - POST" />
          <hr />
                ...
                <TabContent>
                  <TabpaneForm id="programming">
                    ...
                    <span>
                      ...
                      <input name="icon" type="text" className="form-control" />
                    </span>
                  ...
                  <TabpaneForm id="educations">
                    <span>
                      <label htmlFor="image">School Picture Url : </label>{" "}
                      <input
                        name="image"
                        type="text"
                    ...
                    <span>
                      <label htmlFor="periode">Periode (xxxx - xxxx) : </label>{" "}
                      <input
                        name="periode"
                        type="text"
                        className="form-control"
                  ...
                  <TabpaneForm id="workExperiences">
                    ...
                    <span>
                      <label htmlFor="jobdesk">Job / Position : </label>{" "}
                      <input
                        name="jobdesk"
                        type="text"
                        className="form-control"
                  ...
                  <TabpaneForm id="certificateSeminar">
                    ...
                    <span>
                      <label htmlFor="time">Activity Time(Month Year) : </label>{" "}
                  ...
                  <TabpaneForm id="socialMedia">
                    ...
                    <span>
                      <label htmlFor="urlAccount">Social Media Url : </label>{" "}
                      <input
                        name="urlAccount"
                        type="text"
                        className="form-control"
